...
export default function SyncLogImageMap() {
  ...
  const { data: syncStatusData } = useQuery({
    queryKey: ['sync-status', 'image_map'],
    queryFn: () => api.getSyncStatus('image_map'),
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['admin-sync-log', 'image_map', page, status, startDate, endDate],
    queryFn: () =>
      api.getSyncLog(
        page,
        limit,
  ...
  const deleteAllMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['admin-sync-log', 'image_map'] });
      setStatusMessage({ 
        type: 'success', 
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      queryClient.invalidateQueries({ queryKey: ['sync-status', 'image_map'] });
      queryClient.invalidateQueries({ queryKey: ['admin-sync-log', 'image_map'] });
    }, 10000);
    ...
    if (statusUpdate) {
      ...
      if (statusUpdate.type === 'info') {
        queryClient.setQueryData(
          ['admin-sync-log', 'image_map', page, status, startDate, endDate],
          (oldData: any) => {
