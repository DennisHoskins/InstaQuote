...
export default function SyncLogDropboxLinks() {
  ...
  const linksMutation = useMutation({
    mutationFn: (token: string) => api.triggerCreateLinks('Dennis', token),
    onSuccess: () => {
      setDialogOpen(false);
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        >
          {linksMutation.isPending ? 'Running...' : 'Create Links'}
        </Button>
      </Box>
      ...
      <DropboxTokenDialog
        open={dialogOpen}
        title="Run Create Dropbox Links"
        onClose={() => setDialogOpen(false)}
        onConfirm={handleRunSync}
        isLoading={linksMutation.isPending}
      />
    </Container>
  );
}